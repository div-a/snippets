...
function App() {
  ...
  const addSnippet = async (newSnippetText) => {
    if (
      ...
      (allSnippets.length > 0 &&
        newSnippetText != allSnippets[allSnippets.length - 1].text)
    ) {
      let newSnippet = {
        text: newSnippetText,
        pageId: page.id,
  ...
  useEffect(() => {
    ...
    async function fetchSnippets() {
      if (page?.id) {
        const pageSnippets = await db.Snippet.where("pageId")
          .anyOf([page.id])
          .toArray();
  ...
  const selectPage = async (event) => {
    const selectedPage = allPages.filter(
      (ap) => ap.name === event.target.innerText
    )[0];
  ...
  const onSearchChange = async (event) => {
    ...
    const page = await db.Page.where("name")
      .anyOf([event.target.value])
      .toArray();
    if (page && page.length > 0) {
      const pageSnippets = await db.Snippet.where({
        pageId: page[0]?.id,
      }).toArray();
      setAllSnippets(pageSnippets);
